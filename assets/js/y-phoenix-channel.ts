 ...
 */
const messageHandlers: ((encoder:encoding.Encoder, decoder: decoding.Decoder, PhoenixChannelProvider: PhoenixChannelProvider, emitSynced: boolean,    messageType: number)=>void)[] = []
 ...
 */
const readMessage = (provider: PhoenixChannelProvider, buf: Uint8Array, emitSynced: boolean):encoding.Encoder => {
  ...
  if (provider.shouldConnect && provider.channel == null) {
    ...
    provider.channel.join().receive("ok", (_resp) => {
      ...
      if (provider.awareness.getLocalState() !== null) {
        ...
        provider.channel?.push("yjs",encoding.toUint8Array(encoderAwarenessState).buffer)
      }
 ...
 */
const broadcastMessage = (provider: PhoenixChannelProvider, buf: Uint8Array) => {
...
export class PhoenixChannelProvider extends Observable {
  ...
  messageHandlers: ((encoder: encoding.Encoder, decoder: decoding.Decoder, PhoenixChannelProvider: PhoenixChannelProvider, emitSynced: boolean, messageType: number) => void)[]
  _synced: boolean
